...
def add_pre_exposures(exposures: Cohort, followup: Cohort, delay: int) -> Cohort:
    ...
    pre_exposures_events = pre_exposures_events.withColumn(
        "start", sf.date_sub("old_start", int(delay))
    )
    pre_exposures_events = pre_exposures_events.withColumn(
        "value", sf.concat(sf.lit("pre-"), sf.col("old_value"))
    )
    ...
    pre_exposures_events = pre_exposures_events.where(is_valid).select(
        *pre_exposures_events_cols
    )
...
def min_exposures(exposures: Cohort, minimum_count: int) -> list[str]:
    return (
        exposures.events.groupBy("value")
        .count()
        .where(sf.col("count") > minimum_count)
        .toPandas()
        .value.values.tolist()
    )


def min_exposures_filter(exposures: Cohort, minimum_count: int) -> Cohort:
    exposures_names = min_exposures(exposures, minimum_count)

    exposure_events = exposures.events.where(sf.col("value").isin(exposures_names))
    subjects = exposure_events.select("patientID")
    return Cohort(
        exposures.name,
        "{} with exposures with at least {} exposures.".format(
            exposures.characteristics, minimum_count
        ),
        subjects,
        exposure_events,
    )
...
def main():
    ...
    min_exp = min_exposures_filter(exposures.intersection(min_base).cache(), 150).cache()
    ...
    logger.info("Number of samples {}".format(len(censoring)))
